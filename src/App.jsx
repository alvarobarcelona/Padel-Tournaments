...
function App() {
  ...
  const [tournamentData, setTournamentData] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : { mode: 'americano', players: [], pointsPerMatch: 24, courtNames: {} };
  });
  ...
  const finishTournament = (finalPlayers) => {
        ...
        <div className="container">
          ...
          {history.length > 0 && (
            <div style={{ marginTop: '3rem' }}>
              <h3 style={{ color: 'var(--text-muted)', marginBottom: '1rem' }}>Tournament History</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {history.map(record => (
                  ...
                  >
                    <div>
                      <div style={{ fontWeight: 'bold' }}>{record.winner} 🏆</div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                        {record.date} • <span style={{ textTransform: 'capitalize' }}>{record.mode}</span>
                      </div>
                    </div>
          ...
          {viewHistoryItem && (
            <div style={{
              ...
              <Standings players={viewHistoryItem.players} onClose={() => setViewHistoryItem(null)} />
            </div>
          )}
        </div>
      ) : (
